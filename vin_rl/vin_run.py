...
def main():
    ...
    parser.add_argument('--num_validation_episodes', type=int, default=200,
    ...
    parser.add_argument('--save_path', help='path to save data in', 
                        type=str, default='./')
    parser.add_argument('--enable_save', help='enables/disables data save', 
                        action='store_true')
    ...
    def evaluate_agent(tr_episode=0):
        avg_reward = 0.
        map_args['train'] = False
        env.set_map_args(map_args)

        toggle_plot = args.enable_save
        ...
        for episode in range(args.num_validation_episodes):
            ...
            for t in range(args.max_episode_length):
                if args.eval:
                    print coord
                action = select_action(grid, coord, infer=True, plot=toggle_plot,
                    save=args.enable_save, save_path=args.save_path, save_id=tr_episode)
                toggle_plot = False
        ...
        if args.enable_save:
            writer.writerow([tr_episode, avg_reward, map_args['difficulty']])
    ...
    def select_action_ac(grid, coord, plot=False, print_info=False, infer=False,
        save=False, save_path=None, save_id=0):
        ''' Actor-Critic'''
        probs, state_value = net(Variable(torch.unsqueeze(grid, 0)),
                                 Variable(torch.unsqueeze(coord, 0)), plot=plot, save=save, save_path=save_path, save_id=save_id)
