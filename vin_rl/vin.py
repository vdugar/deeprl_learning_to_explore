...
class VIN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fc1 = nn.Linear(in_features=args.ch_q,  # After attention model -> Q(s, .) for q layers
                             out_features=args.num_actions,  # 8 available actions
                             bias=False)
    ...
    def forward(self, X, C, record_images=False, plot=False, save=False, save_path=None,
        save_id=0):
        ...
        # if print_stuff:
        #    scipy.misc.imsave('grid'+str(self.counter)+'.jpg', X.data[0].cpu().numpy()[0])
        #    scipy.misc.imsave('goal'+str(self.counter)+'.jpg', X.data[0].cpu().numpy()[1])
        #    scipy.misc.imsave('reward'+str(self.counter)+'.jpg', r.data[0].cpu().numpy()[0])
        ...
        if plot:
           ...
           #saveplot with current time
           fig.savefig('save/'+str(datetime.now())+'.jpg')
           #plt.show()
           plt.draw()
           plt.ion()
           plt.show()
        ...
        # Attention model
        q_out = attention(q, [C[:, 0].long(), C[:, 1].long(), self.args])
