...
class Sample:
    ...
    def isTerminal(self):
        '''
        Returns if the action finished the episode
        '''
...
class ReplayMemory:
    ...
    def append(self, state, action, reward, next_state, is_terminal):
        self.memory[self.index] = Sample(state, action, reward, next_state, is_terminal)
    ...
    def sample(self, batch_size, indexes=None):
        ...
        if indexes is None:
            if self.full:
                sampled_indices = np.random.randint(0, self.max_size, batch_size)
            else:
                sampled_indices = np.random.randint(0, self.index, batch_size)
        else:
