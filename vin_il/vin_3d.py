...
def attention(tensor, params):
    ...
    slice_s1 = S1.expand(args.ch_q, 1, args.imsize, args.imsize, num_data)
    # slice_s1 = S1.expand(num_data, args.ch_q, 1, args.imsize, args.imsize)
    slice_s1 = slice_s1.permute(4, 0, 1, 2, 3)
    q_out = tensor.gather(2, slice_s1).squeeze(2)

    # Slicing S2 positions
    slice_s2 = S2.expand(args.ch_q, 1, args.imsize, num_data)
    # slice_s2 = S2.expand(num_data, args.ch_q, 1, args.imsize)
    slice_s2 = slice_s2.permute(3, 0, 1, 2)
    q_out = q_out.gather(2, slice_s2).squeeze(2)

    # Slicing S3 positions
    slice_s3 = S3.expand(args.ch_q, 1, num_data)
    # slice_s3 = S3.expand(num_data, args.ch_q, 1)
    slice_s3 = slice_s3.permute(2, 0, 1)
...
class VIN(nn.Module):
    ...
    def forward(self, X, S1, S2, S3, args, record_images=False):
        ...
        # Attention model
        q_out = attention(q, [S1.long(), S2.long(), S3.long(), args])
